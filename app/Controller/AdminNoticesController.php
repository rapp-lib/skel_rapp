 ...
 * @controller
 */
class AdminNoticesController extends Controller_Admin
{
    /**
     * 検索フォーム
     */
    protected static $form_search = array(
        "receive_all" => true,
        "search_page" => "admin_notices.list",
        "search_table" => "Notice",
        "fields" => array(
    ...
    {
        $this->vars["t"] = table("Notice")->selectById($this->input["id"]);
    ...
    {
        if ($id = $this->input["id"]) {
            table("Notice")->deleteById($id);
        }
        return $this->redirect("id://admin_notices.list", array("back"=>"1","complete_flg"=>"delete"));
    }
}
