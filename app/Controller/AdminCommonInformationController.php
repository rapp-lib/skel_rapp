 ...
 * @controller
 */
class AdminCommonInformationController extends Controller_Admin
{
    /**
     * 検索フォーム
     */
    protected static $form_search = array(
        "receive_all" => true,
        "search_page" => "admin_common_information.list",
        "search_table" => "CommonInformation",
        "fields" => array(
    ...
    protected static $form_entry = array(
        "form_page" => "admin_common_information.form",
        "csrf_check" => true,
        "table" => "CommonInformation",
        "fields" => array(
            "id"=>array("label"=>"ID"),
            "file"=>array("label"=>"ファイル", "storage"=>"public"),
            "title"=>array("label"=>"タイトル"),
    ...
    {
        if ($id = $this->input["id"]) {
            table("CommonInformation")->deleteById($id);
        }
        return $this->redirect("id://admin_common_information.list", array("back"=>"1"));
    }
}
